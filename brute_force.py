...
def generate_strings() -> generator:
    """
    This generator generates weak passwords consisting of ASCII letters and digits up to 8 characters long.
    Source: Chat GPT.
    """
...
def brute_force() -> None:
    ...
    if user is None:
        _user = app.create_user(email="victim@example.com", password="b")

    # Let's say the attacker knows the target's email address, but not the password.
    # Due to the weak password policy and no API rate limiting, the hacker can crack the password using a brute force
    # attack.
